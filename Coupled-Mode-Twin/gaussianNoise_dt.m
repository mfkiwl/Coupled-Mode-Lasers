...
function [tout, Nout] = gaussianNoise_dt(tsim, dt, QA, QB, etaAB, etaBA, theta, DW, param, varargin)
%
%% Description
%
%%  Usage
%
%   [tout, Nout] = gaussianNoise_dt(tsim, dt, QA, QB, etaAB, etaBA, theta, DW, param, varargin)
%
%%  Arguments
%
...
%
%   DT          integration step size (pass 0 to use variable step routine)
%
...
%% Check RK4 routine is on the path
...
%% Set up Gaussian noise sources
    ...
    % Period of the fluctuations (use time step if specified)
    if (dt > 0)
...
%     [warnMsg, ~] = lastwarn;
    ...
    if (dt > 0)
        ...
        disp('')
        disp('USING FIXED STEP RK ROUTINE')
        disp(['dt = ' num2str(dt) ' ns'])
        disp('')
        ...
        disp('')
        disp('USING VARIABLE STEP RK ROUTINE')
        ...
        if (dt < 0)
            ...
            % Used to set maximum step for ode45
            maxt = abs(dt);
            options = odeset('MaxStep', maxt);    % Impose maximum step size on ode45
            disp(['Imposing maximum step size: dt = ' num2str(maxt) ' ns'])
